...
public class TeleporterKether extends Teleporter {
   private final WorldServer worldServerInstance;
   private final Random random;
   private final LongHashMap destinationCoordinateCache = new LongHashMap();
   private final List destinationCoordinateKeys = new ArrayList();

   public TeleporterKether(WorldServer p_i1963_1_) {
      super(p_i1963_1_);
         ...
         for(int l = -2; l <= 2; ++l) {
            for(int i1 = -2; i1 <= 2; ++i1) {
               for(int j1 = -1; j1 < 3; ++j1) {
                  ...
                  int i2 = k + i1 * b1 - l * b0;
                  boolean flag = j1 < 0;
                  this.worldServerInstance.setBlock(k1, l1, i2, flag ? KMOD_Main.MagicStone : Blocks.air);
               }
            }
         }
   ...
   public boolean placeInExistingPortal(Entity p_77184_1_, double p_77184_2_, double p_77184_4_, double p_77184_6_, float p_77184_8_) {
      ...
      if (this.destinationCoordinateCache.containsItem(j1)) {
         TeleporterKether.PortalPositionKether portalposition = (TeleporterKether.PortalPositionKether)this.destinationCoordinateCache.getValueByKey(j1);
         d3 = 0.0D;
         ...
         for(l3 = l - short1; l3 <= l + short1; ++l3) {
            ...
            for(int l1 = i1 - short1; l1 <= i1 + short1; ++l1) {
               ...
               for(j2 = this.worldServerInstance.getActualHeight() - 1; j2 >= 0; --j2) {
                  if (this.worldServerInstance.getBlock(l3, j2, l1) == KMOD_Main.KetherPortal) {
                     while(this.worldServerInstance.getBlock(l3, j2 - 1, l1) == KMOD_Main.KetherPortal) {
                        --j2;
                     }
      ...
      if (d3 >= 0.0D) {
         if (flag) {
            this.destinationCoordinateCache.add(j1, new TeleporterKether.PortalPositionKether(i, j, k, this.worldServerInstance.getTotalWorldTime()));
         ...
         d7 = (double)k + 0.5D;
         int i4 = -1;
         if (this.worldServerInstance.getBlock(i - 1, j, k) == KMOD_Main.KetherPortal) {
            i4 = 2;
         }
         ...
         if (this.worldServerInstance.getBlock(i, j, k - 1) == KMOD_Main.KetherPortal) {
            i4 = 3;
         }
         ...
         for(i3 = -1; i3 <= 1; ++i3) {
            for(j3 = 1; j3 < 3; ++j3) {
               for(k3 = -1; k3 < 3; ++k3) {
                  ...
                  j4 = k2 + (j3 - 1) * l2 - i3 * l5;
                  flag = k3 < 0;
                  this.worldServerInstance.setBlock(l3, i4, j4, flag ? KMOD_Main.MagicStone : Blocks.air);
               }
            }
         }
      }
      ...
      for(i3 = 0; i3 < 4; ++i3) {
         for(j3 = 0; j3 < 4; ++j3) {
            for(k3 = -1; k3 < 4; ++k3) {
               ...
               this.worldServerInstance.setBlock(l3, i4, j4, flag ? KMOD_Main.MagicStone : KMOD_Main.KetherPortal, 0, 2);
            }
         }
   ...
   public void removeStalePortalLocations(long p_85189_1_) {
      if (p_85189_1_ % 100L == 0L) {
         ...
         while(true) {
            Long olong;
            TeleporterKether.PortalPositionKether portalposition;
            do {
               if (!iterator.hasNext()) {
                  return;
               }
               ...
               portalposition = (TeleporterKether.PortalPositionKether)this.destinationCoordinateCache.getValueByKey(olong);
