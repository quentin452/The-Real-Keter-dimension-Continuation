...
public abstract class KetherGenHugeTreeBaseNormal extends KetherAbstractTree {
   protected final int baseHeight;
   protected final int woodMetadata;
   protected final int leavesMetadata;
   protected int field_150538_d;

   public KetherGenHugeTreeBaseNormal(boolean p_i45458_1_, int p_i45458_2_, int p_i45458_3_, int p_i45458_4_, int p_i45458_5_) {
      super(p_i45458_1_);
   ...
   private boolean func_150532_c(World p_150532_1_, Random p_150532_2_, int p_150532_3_, int p_150532_4_, int p_150532_5_) {
      p_150532_1_.getBlock(p_150532_3_, p_150532_4_ - 1, p_150532_5_);
      boolean isSoil = true;
      if (isSoil && p_150532_4_ >= 2) {
         this.onPlantGrow(p_150532_1_, p_150532_3_, p_150532_4_ - 1, p_150532_5_, p_150532_3_, p_150532_4_, p_150532_5_);
         this.onPlantGrow(p_150532_1_, p_150532_3_ + 1, p_150532_4_ - 1, p_150532_5_, p_150532_3_, p_150532_4_, p_150532_5_);
         this.onPlantGrow(p_150532_1_, p_150532_3_, p_150532_4_ - 1, p_150532_5_ + 1, p_150532_3_, p_150532_4_, p_150532_5_);
         this.onPlantGrow(p_150532_1_, p_150532_3_ + 1, p_150532_4_ - 1, p_150532_5_ + 1, p_150532_3_, p_150532_4_, p_150532_5_);
         return true;
      } else {
         return false;
      }
         ...
         for(int l1 = p_150535_4_ - p_150535_5_; l1 <= p_150535_4_ + p_150535_5_ + 1; ++l1) {
            ...
            if (k1 * k1 + i2 * i2 <= i1 || j2 * j2 + k2 * k2 <= i1 || k1 * k1 + k2 * k2 <= i1 || j2 * j2 + i2 * i2 <= i1) {
               ...
               if (block.isAir(p_150535_1_, j1, p_150535_3_, l1) || block.isLeaves(p_150535_1_, j1, p_150535_3_, l1)) {
                  this.setBlockAndNotifyAdequately(p_150535_1_, j1, p_150535_3_, l1, KMOD_Main.KetherLeaves, this.leavesMetadata);
               }
            }
         }
      }
         ...
         for(int l1 = p_150534_4_ - p_150534_5_; l1 <= p_150534_4_ + p_150534_5_; ++l1) {
            ...
            if (k1 * k1 + i2 * i2 <= i1) {
               ...
               if (block.isAir(p_150534_1_, j1, p_150534_3_, l1) || block.isLeaves(p_150534_1_, j1, p_150534_3_, l1)) {
                  this.setBlockAndNotifyAdequately(p_150534_1_, j1, p_150534_3_, l1, KMOD_Main.KetherLeaves, this.leavesMetadata);
               }
            }
         }
      }
